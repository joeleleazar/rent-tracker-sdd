...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        <div class="d-flex flex-column flex-md-row min-vh-100" hx-boost="true">
            {{--
                Sidebar de navegación (Principio III: Senior-First) — vertical y
                siempre visible en escritorio (sin acordeón ni menú oculto detrás
                de un botón "hamburguesa"); en pantallas angostas se reordena a una
                franja horizontal con los mismos enlaces, sin ocultar ninguno.
            --}}
            <nav
                ...
                <div class="d-flex flex-md-column flex-row flex-wrap align-items-center align-items-md-stretch justify-content-between gap-3">
                    ...
                    @auth
                        <ul class="nav nav-pills flex-md-column flex-row flex-wrap gap-2 mb-md-3" style="min-width: 0;">
                            <li class="nav-item">
                                <a href="{{ route('dashboard') }}" class="nav-link text-white d-flex align-items-center gap-2 fs-5 py-3 {{ request()->routeIs('dashboard') ? 'active' : '' }}">
                                    <i class="bi bi-buildings" aria-hidden="true"></i> Locaciones
                                </a>
                            </li>
                            <li class="nav-item">
                                <a href="{{ route('locaciones.index') }}" class="nav-link text-white d-flex align-items-center gap-2 fs-5 py-3 {{ request()->routeIs('locaciones.*') ? 'active' : '' }}">
                                    <i class="bi bi-diagram-3" aria-hidden="true"></i> Gestionar Locaciones
                                </a>
                            </li>
                            <li class="nav-item">
                                <a href="{{ route('configuracion.edit') }}" class="nav-link text-white d-flex align-items-center gap-2 fs-5 py-3 {{ request()->routeIs('configuracion.*') ? 'active' : '' }}">
                        ...
                        <div class="d-flex flex-md-column flex-row align-items-center align-items-md-stretch gap-3 mt-md-auto pt-md-3 border-top border-secondary">
                            <span class="fs-5 text-white-50">{{ Auth::user()->name }}</span>
