    ...
    <div class="col-12 col-lg-8" style="max-width: 42rem;">
        ...
        <form method="POST" action="{{ route('locaciones.store') }}" class="d-flex flex-column gap-4">
            ...
            <div>
                ...
                <x-text-input id="nombre" name="nombre" type="text" :value="old('nombre')" required />
                <x-input-error :messages="$errors->get('nombre')" class="mt-2" />
            </div>

            <div>
                <x-input-label for="tamano" value="Tamaño (m²)" />
                <x-text-input id="tamano" name="tamano" type="number" step="0.01" min="0.01" :value="old('tamano')" required />
                <x-input-error :messages="$errors->get('tamano')" class="mt-2" />
            </div>

            <div>
                <x-input-label for="ubicacion_fisica" value="Ubicación física" />
                <x-text-input id="ubicacion_fisica" name="ubicacion_fisica" type="text" :value="old('ubicacion_fisica')" required />
            ...
            <div>
                ...
                <select id="locacion_padre_id" name="locacion_padre_id" class="form-select">
                    ...
                    @foreach ($locaciones as $opcion)
                        <option value="{{ $opcion->id }}" @selected(old('locacion_padre_id', $locacionPadreId) == $opcion->id)>
                            {{ $opcion->nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="d-flex flex-wrap gap-3">
                ...
                <a href="{{ route('locaciones.index') }}" class="btn btn-outline-secondary"><i class="bi bi-x-lg" aria-hidden="true"></i> Cancelar</a>
            </div>
        </form>
    </div>
</x-layouts.app-bootstrap>
