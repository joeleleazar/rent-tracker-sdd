...
@if (session('contratoEnConflicto'))
    @php
        ...
        $inquilinoNuevo = ($inquilinos ?? collect())->firstWhere('id', (int) old('inquilino_id'));
    @endphp

    <x-modal-bootstrap name="modal-solapamiento" :show="true" maxWidth="lg" focusable>
        <div class="modal-header border-danger border-3">
        ...
        <div class="modal-body p-4 fs-5">
            ...
            <div class="alert alert-warning" role="alert">
                ...
                <i class="bi bi-person" aria-hidden="true"></i> Inquilino: {{ $inquilinoNuevo->nombre ?? '—' }}<br>
        ...
        <div class="modal-footer">
            <button type="button" class="btn btn-outline-secondary btn-lg" data-bs-dismiss="modal">Cerrar</button>
            <a href="{{ route('contratos.edit', $existente['contrato_id']) }}" class="btn btn-warning btn-lg">Editar Contrato Existente</a>
        </div>
    </x-modal-bootstrap>
@endif
