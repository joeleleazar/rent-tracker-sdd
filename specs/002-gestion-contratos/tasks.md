...
- [X] T013 [P] Crear layout Blade base accesible (tipografía ≥18px, tokens de contraste WCAG AA/AAA, utilidades de botón ≥48x48px, navegación plana) en `resources/views/layouts/app.blade.php` — más `resources/css/app.css` (html a 18px base + utilidades `.btn-senior-*`/`.campo-senior`) y componentes base de Breeze (`primary-button`, `secondary-button`, `danger-button`, `text-input`, `input-label`, `input-error`) migrados a estas clases; componentes de dropdown eliminados (Senior-First prohíbe menús desplegables)
...
- [X] T023 [US1] Crear vista `contratos.create` (formulario Senior-First) en `resources/views/contratos/create.blade.php`
