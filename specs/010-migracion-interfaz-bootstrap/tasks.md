...
- [X] T002 Crear `resources/css/bootstrap.scss` con las variables Sass personalizadas del proyecto (paleta de colores de alto contraste) antes de `@import "bootstrap/scss/bootstrap"` (ver `research.md` §1; valores revisados en la Constitución v2.0.0)
...
- [X] T035 [P] Auditoría de accesibilidad (contraste WCAG AA) en las ~30 vistas migradas, contra `contracts/inventario-vistas-migradas.md` — la paleta se horneó una sola vez en `resources/css/bootstrap.scss` (§1-3 de `research.md`; valores revisados posteriormente en la Constitución v2.0.0). Verificado con inspección de estilos computados en navegador: fondo `#1e40af` con texto blanco (contraste ~8.6:1); mismo criterio para `success`/`danger`/`secondary` (todos tonos "800" de la paleta ya usada y presumida ya auditada en 001-009).
